"""D1H-specific observation terms.

Ported from DDT_Lab: source/ddt_lab/ddt_lab/tasks/manager_based/locomotion/mdp/observations.py
"""
...
def contact_state(
    ...
    Shape: ``(num_envs, num_feet)``. Mirrors reference ``contact_filt.float() - 0.5``.
    """
...
def joint_kp_factor(
    ...
    Shape: ``(num_envs, num_joints)``. Mirrors reference ``kp_factor`` in priv_latent;
    values outside [0, 2] are clamped to avoid very large signals from near-zero defaults.
    """
    asset: Articulation = env.scene[asset_cfg.name]
...
def joint_kd_factor(
    ...
    Shape: ``(num_envs, num_joints)``. Mirrors reference ``kd_factor`` in priv_latent.
    """
    asset: Articulation = env.scene[asset_cfg.name]
