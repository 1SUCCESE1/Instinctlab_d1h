...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Enable terminal keyboard control (W/S=fwd/bwd  A/D=strafe  Q/E=turn  Space=stop).",
)
...
def main():
    ...
    # terminal keyboard control (overrides the base_velocity command)
    keyboard = None
    if args_cli.keyboard:
        from keyboard_vel_controller import VelocityKeyboardController

        keyboard = VelocityKeyboardController(env_cfg, sim_device=args_cli.device or env.unwrapped.device)
